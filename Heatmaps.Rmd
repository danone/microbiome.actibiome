...
heatmap_phylo <- function(category, x, order){
  ...
  phy <- merge_samples(physeq_norm, category)

  phy_normalized  <- transform_sample_counts(phy, function(x) log10(x/mean(x)))
  ...
  p$labels$fill <- expression(log[10]('Relative Abundance'))
  p$labels$x <- x
