...
heatmap_phylo <- function(category, x, order, label_title){
  ...
  phy_normalized  <- transform_sample_counts(physeq_norm, function(x) x / sum(x))
  
  phy_normalized <- merge_samples_mean(phy_normalized, category)
...
heatmap_phylo("gender", "Age Category", category_gender, expression(log[10]*'(Relative Mean Abundance)'))
...
## on va calculer les prévalences cette fois-ci :
...
mapp <- data.frame(category = category_age, fill = category_age) %>%
  column_to_rownames(.,var='category')
...
#phy  <- transform_sample_counts(phy, function(x) log10(x))

p <- plot_heatmap(phy,
                  taxa.label = "Species", taxa.order = "Species", 
                  low="beige", high="red", na.value="beige",sample.order = category_age)
