...
plot('westernized','Patients Counts by Westernized', 'Westernized')
...
p<-ggboxplot(map_adult_no_antibio, x="westernized", y="reads_bifid", fill="westernized",outlier.shape = NA )
...
p <- p +
  ...
  labs(x = NULL, y = NULL, title = "Relative Abundance of Bifidobacterium among Antibiotic non-takers",fill= "Westernized") +
...
p<-ggplot(map_age, aes(x = age_category,
  ...
  geom_boxplot(outlier.shape = NA) + labs(fill= "Age category")
  
p <- p + scale_fill_brewer(palette="Dark2", labels = c("newborn (137)","child (160)","schoolage (88)","Adult (4921)", "Senior #(422)")) +  
  
  theme_minimal() + labs(x = NULL, y = NULL, title = "Relative Abundance of Bifidobacterium, no Antibiotics Takers") + labs(color= "Age Category") +
  coord_cartesian(ylim=c(0,0.40)) + stat_compare_means(label.y = 0.35, label.x ="child") +
