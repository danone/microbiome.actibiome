...
# on filtre le patient SID32.PR.V.1 qui pose problème pour la suite de l'analyse

otus <- otus %>%
  select(-SID32.PR.V.1)

map <- map %>%
  filter(sample_id != 'SID32.PR.V.1')
...
# on save les fichiers après QC

save(map, otus, tax, file="study_after_QC.rda")
