...
which_study <- readline(prompt=paste("Do you want to cluster the data on ASNICAR (Predict) or whole CURATED data without predict ? : ", "pre - PREDICT", "cur - CURATED minus PREDICT","who - WHOLE CURATED DATA SET","", sep="\n"))

while(!which_study %in% c("cur", "pre", "who")){
  which_study <- readline(prompt=paste("Error, please redo : ", "pre - PREDICT", "cur -
                                       CURATED minus PREDICT","who - WHOLE CURATED DATA SET", "", sep="\n"))
}
...
write.csv(enterotype, file="enterotype_DMM_adults", row.names=TRUE, col.names=TRUE, quote=FALSE)
...
top_5_ab <- matrix_int %>%
  data.frame(.)
colnames(top_5_ab) <- c( 1:(as.numeric(max(enterotype$bifidotype)) -1))

rownames(top_5_ab) = gsub("B.", "Bifidobacterium", rownames(top_5_ab))

## we select the commensal + animaliss species

species_to_select <- c("Bifidobacterium  animalis","Bifidobacterium  adolescentis","Bifidobacterium  pseudocatenulatum","Bifidobacterium  dentium","Bifidobacterium  longum","Bifidobacterium  bifidum","Bifidobacterium  catenulatum","Bifidobacterium  angulatum")
...
p  <- ggplot(sam_data, aes(x = bifidotype,
                       y = bif_rel, fill=bifidotype))+ geom_boxplot(outlier.shape = NA) 
