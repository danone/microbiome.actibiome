...
bifidotype <- enterotype %>%
  filter(sample_id %in% adults$sample_id) #%>% filter(sample_id %in% ascinar$sample_id)
...
colnames(matrix_int) <- as.character(paste0('cluster',1:6))

top_5 <- matrix_int %>%
  data.frame(.) %>%
  rownames_to_column('species') %>%
  gather(key="cluster", value="value", 2:7) %>%
  group_by(cluster) %>%
  top_n(5, value) %>%
  select(-value) %>%
  rename('cluster' = 'bifidotype')
