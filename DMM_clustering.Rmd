...
if(!(dmm_name %in% list.files("./DMM_files/")) & !(seeds_list_name %in% list.files("./DMM_files/"))) {

  for(i in 1:5) {

    set.seed(seeds[i])

    fit_genus <- parallel::mclapply(1:30, dmn, count=count, verbose=FALSE, mc.cores=12)
...
number_of_clusters <- as.character(length(fit_genus_list[[1]]))
...
sd_rank <- test_unfactored %>%
  ...
  #summarise_all(mean) %>%
  gather(., key="k", value="sd_rank", '1':as.character(number_of_clusters))
...
merge(mean_rank, sd_rank, by=c("method","k")) %>%
  filter(method=="bic") %>%

  ggplot() +
    geom_line(aes(x=k, y=mean_rank, group=method, color =method)) +
  #geom_line(aes(x=k, y=mean_rank))
    #geom_line(aes(x=k, y=mean_rank + (1.96*(sd_rank/sqrt(30))), group=method, color =method, linetype=2)) +
    geom_ribbon(aes(x=k, ymin = mean_rank - (1.96*(sd_rank/sqrt(30))), ymax = mean_rank + (1.96*(sd_rank/sqrt(30))),  group=method, fill =method), alpha = 0.2) +
  #geom_ribbon(aes(x=k, ymin = mean_rank - (1.96*(sd_rank/sqrt(30))), ymax = mean_rank + (1.96*(sd_rank/sqrt(30)))), alpha = 0.2) +
    theme_classic() +
    scale_x_discrete("Number of Dirichlet component",limits = as.character(1:number_of_clusters)) +
    scale_color_brewer("criteria",type="qual") +
    scale_fill_brewer("criteria",type="qual") +
    ylab("model optimum average rank") + guides(color=FALSE,fill=FALSE) -> criteria_dmm_plot

save(criteria_dmm_plot, file="criteria_dmm_plot.rda")

criteria_dmm_plot

ggsave(filename = "criteria_dmm_plot.pdf" )
...
fit_genus_k1 <- parallel::mclapply(1:1, dmn, count=count, verbose=FALSE, mc.cores=12)
...
distr_clusters <- ggplot(distribution, aes(x = ., y = Freq, fill= .)) +  # Plot with values on top
  ...
  geom_text(aes(label = Freq, vjust=1.5),
            size=5,
            color="white") +
  labs( y = "Subject count", x = "Bifidobacterium based gut microbiome partitions") +
  theme_classic() +
  scale_fill_brewer("", type="qual", palette=2) +
  scale_x_discrete(limits = as.character(1:max(distribution$.))) +
  ylab("number of individuals") +
  guides(fill=FALSE) + annotate("text", x=7, y=700, label="(undetected\nbifid\npartition)", size=4)
...
colnames(matrix_int) <- paste("partitions ",1:ncol(matrix_int))
...
heatmap.2(matrix_int,
          Colv=FALSE,   # do not cluster partitions
  ...
  key.xlab = "") -> bif_DMM_alpha_weight_df
...
bif_DMM_alpha_weight_df$carpet %>%
  reshape2::melt() %>%
  ggplot() + geom_tile(aes(x=Var1,y=Var2,fill=10^value)) +
  geom_text(aes(x=Var1,y=Var2,label=round(10^value,4))) +
  scale_fill_viridis_c("DMM model\nalpha weights", trans="log10") +
  xlab("Bifidobacterium based gut micrbiome partitions") + 
  ylab(expression(paste(italic("Bifidobacterium"), " species"))) +
  theme_classic() +
  theme(axis.text.y = element_text(face="italic")) -> bif_DMM_alpha_weight_plot
 
bif_DMM_alpha_weight_plot
...
bif_DMM_alpha_weight_plot) + plot_annotation(tag_levels = "A")
...
p_relative_abundance <- ggplot(sam_data, aes(x = bifidotype,
                       y = bif_rel+10^-5, fill=bifidotype))+
  ...
  ylab("Bifidobacterium genus proportion") +
  xlab("Bifidobacterium based gut microbiome partitions")
...
pairwise.wilcox.test(sam_data$bif_rel, sam_data$bifidotype) %>% 
  broom::tidy() %>%
  mutate(fdr=p.adjust(p.value, method="fdr")) %>%
  readr::write_tsv(file = "bif_rel_pairwise_wilcox.tsv")
...
p_diversity2 <- ggplot(diversity, aes(x = bifidotype,
                       y = Shannon, fill=bifidotype)) +
  geom_boxplot(outlier.shape = NA) +
  theme_classic() +
  stat_compare_means(label.y = 4.5, label.x ="3") +
  ylab("Alpha-diversity (Shannon)" ) +
  xlab("Bifidobacterium based gut microbiome partition") +
  scale_fill_brewer(type="qual", palette = 2) +
  guides(fill=FALSE)
  #labs(x = "Bifidotype", y = "", title = "Shannon diversity",color= "") + 
  #coord_cartesian(ylim=c(0.8,4.5)) +
...
pairwise.wilcox.test(diversity$Shannon, diversity$bifidotype) %>% 
  broom::tidy() %>%
  mutate(fdr=p.adjust(p.value, method="fdr")) %>%
  readr::write_tsv(file = "bif_shannon_pairwise_wilcox.tsv")
...
(p_relative_abundance/p_diversity2) + plot_annotation(tag_levels = "A")
...
distribution <- function(category, type){
  ...
  map <- map %>%
    mutate(category = factor(category, levels=c("newborn","child","schoolage","adult","senior"))) }
    ...
    labs(x = "Cluster K", y = "percent", fill = category) +
      #guides(y = "none") +
      ylim(0, 1.2)# + scale_y_continuous("prevalence per partition", label=scales::percent())
...
distr_disease <- distribution('disease', "qual") +
  labs(x = "Bifidobacterium based gut microbiome partitions", y = "", fill= "disease") +
  coord_cartesian(ylim=c(0,1.02)) + scale_fill_brewer("Health status", type="qual")
...
distr_age <- distribution('age_category', "qual") +
  labs(x = "Bifidobacterium based gut microbiome partitions",fill= "age category") +
  coord_cartesian(ylim=c(0,0.95)) +  scale_fill_brewer("Age category", type="div", palette = 7)
...
distr_westernized <- distribution('westernized', "qual") +
  labs(x = "Bifidobacterium based gut microbiome partitions", fill= "westernized") +
  coord_cartesian(ylim=c(0,1.03)) +  scale_fill_brewer("Westernized\nlifestyle", type="qual", palette = 4)
...
(distr_age + scale_y_continuous("prevalence per partition", label=scales::percent) |
distr_westernized + scale_y_continuous("prevalence per partition", label=scales::percent, limits = c(0,1)) |
distr_disease + scale_y_continuous("prevalence per partition", label=scales::percent, limits = c(0,1))) +
  plot_annotation(tag_levels = "A")

ggsave("figure_distributions.pdf")
...
healthy_healthy <- readxl::read_excel('metadata_healthy_healthy.xlsx') %>%
  data.frame(.) %>%
...
table(test_join$assigned_bifidotype,test_join$bifidotype) 

```



## bifidotype per study
...
sampleMetadata %>% 
  ...
  merge(bifidotypes, by="sample_id") %>% 
  select(study_name,bifidotype) %>% 
  group_by(study_name) %>%
  mutate(n=n()) %>%
  filter(n>500) %>%
  select(study_name,bifidotype) %>%
  ...
  prop.table(1) %>%
  reshape2::melt() %>%
  mutate(study_name = ifelse(study_name=="LifeLinesDeep_2016","ZhernakovaA_2016",as.character(study_name))) %>%
  ggplot() + 
  geom_bar(aes(x=as.character(bifidotype), y=value, fill=study_name), stat="identity", position = "dodge") +
  theme_classic() +
  scale_y_continuous("% of individuals",labels=scales::percent) +
  xlab("Bididobacterium-based gut microbiome partitions") +
  scale_fill_brewer("", type="qual", palette = 3)
  

ggsave("partitions_bif_studies.pdf")

```


# sequencing depth vs partitions

```{r}

sampleMetadata %>% 
  select(number_reads, sample_id) %>% 
  merge(bifidotypes, by="sample_id") %>% 
  select(bifidotype, number_reads) %>%
  with(., pairwise.wilcox.test(.$number_reads, .$bifidotype, data=.)) %>%
  broom::tidy() %>%
  mutate(fdr=p.adjust(p.value,method="fdr")) %>%
  readr::write_tsv(file = "bif_number_reads_wilcox.tsv")
