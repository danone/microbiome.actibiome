...
distribution$enterotype <- as.character(distribution$enterotype)

ggplot(distribution, aes(x = enterotype, y = n, fill = enterotype)) +  # Plot with values on top
...
p <- ggplot(diversity, aes(x = enterotype,
  ...
  stat_compare_means(label = "p.signif", method = "t.test", ref.group = "1", label.y = 4.1,) +
...
distribution <- function(category, type){
 ...
 # map$percent <- round(map$percent*100,1)
  ...
  p <- ggplot(map, aes(x = enterotype,y=percent, fill = category)) +
    geom_bar(stat="identity") +
    geom_text(aes(label = paste0(round(percent*100,1),"%, (",rows,")")),position = position_fill(vjust = 0.5), size =2.5) +
