...
mags_ko_table <- mags_tax_gene_id_eggnog %>%
  ...
  mutate(ko_n=n()) %>%
   filter(ko_n>10) %>%
   ungroup() %>%
   group_by_at(vars('genome_name','KEGG_ko')) %>%
   summarise(n=n()) %>%
   mutate(n=ifelse(n>1,2,n)) %>%
   reshape2::dcast(paste0('genome_name', '~', 'KEGG_ko'), value.var = "n", fill=0)
...
for (one_species in species){
   ...
   pheat <- pheatmap(heat_ko %>% filter(row.names(.) %in% rownames(mags_tax_filtered)),
                     ...
                     main= paste0(one_species, " MAGS | ", functions_name, " annotations")
                     )
