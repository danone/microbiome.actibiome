...
#names_otu_interet <- select_n_otu(132, otu_filtered)
...
test <- test %>%
  unfactor(.) %>%
  mutate(bifid = case_when(grepl("Bifidobacterium", V1) ~ V1
                        )) %>%
   mutate(non_bifid = case_when(!grepl("Bifidobacterium", V1) ~ V1
                        ))
